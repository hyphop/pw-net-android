...
public class StreamService extends Service implements Runnable {
  ...
  public int onStartCommand(Intent i, int flags, int id) {
    ...
    // normal start
    resultCode = (i != null) ? i.getIntExtra("code", Activity.RESULT_CANCELED)
  ...
  */
  ...
  public void debug_info(){
      ...
      if (Build.VERSION.SDK_INT >= 26) {
        ...
        if (ch != null)
          Log.i("MiniNativeStream",
                "channel " + CH + " importance=" + ch.getImportance());
      }

      // replace the failing debug with this:
      int round =
          getResources().getIdentifier("ic_launcher_round", "mipmap", getPackageName());
      if (round != 0) {
...
private void logAudioRecordConfig(AudioRecord rec) {
  ...
  public void run() {
    ...
    outer:
    ...
    } catch (Throwable t) { Log.e(TAG, "fatal", t); } finally {
      try {
        if (rec != null) {
          rec.stop();
          rec.release();
      ...
      try {
        if (data != null) {
          MediaProjectionManager mpm =
              (MediaProjectionManager)getSystemService(Context.MEDIA_PROJECTION_SERVICE);
          MediaProjection mp2 = mpm.getMediaProjection(resultCode, data);
          if (mp2 != null) mp2.stop();
        }
      } catch (Throwable ignore) {}
